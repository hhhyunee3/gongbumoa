...
<p class="sub" style="max-width:760px;font-size:14px">결과는 학생의 출발점과 학습량에 따라 달라집니다. 무료 진단에서 우리 아이만의 플랜을 받아보세요.</p>
</div></section>`;
})()}
...
function regionSubjectPage({ sido, sgg, dong, subj, url }) {
  ...
  const howToLd = {
    ...
    description: `${SITE.name}의 ${place} ${subj.name} 과외는 무료 진단, 선생님 안내, 맞춤 수업, 주간 리포트의 4단계로 진행됩니다.`,
    step: [
      { '@type': 'HowToStep', position: 1, name: '무료 진단', text: `첫 수업 전 테스트와 상담으로 ${subj.name} 취약 단원과 학습 성향을 파악합니다.` },
      { '@type': 'HowToStep', position: 2, name: '선생님 안내', text: `진단 결과와 학생 성향에 맞는 ${place} 인근 선생님을 연결합니다. 초반에 맞지 않으면 다른 선생님과 만나보실 수 있습니다.` },
...
<div class="step"><div class="n">2</div><h3>선생님 안내</h3><p>${pick([
  ...
  `진단 결과에 맞는 ${esc(place)} 지역 선생님을 안내해 드립니다.`,
...
function regionGradeSubjectPage({ sido, sgg, dong, subj, grade, url }) {
  ...
  const gradeFaqPool = [
    ...
    { q: `선생님은 어떻게 정해지나요?`, a: `진단 결과와 ${grade.name} 학생 성향, ${place} 지역 일정을 종합해 안내해 드립니다. 수업 초반에 맞지 않는다고 느끼시면 부담 없이 다른 선생님과 만나보실 수 있습니다.` },
  ];
  gradeFaqPool.push(
...
<div class="step"><div class="n">2</div><h3>선생님 안내</h3><p>${pick([`${grade.name} 지도 경험이 있는 ${esc(place)} 선생님을 연결해요.`, `진단 결과와 아이 성향에 맞는 선생님을 안내해 드립니다.`, `${esc(place)} 지역 방문·화상 가능한 선생님을 찾아드려요.`], sd, 91)}</p></div>
...
<p class="sub" style="max-width:760px;font-size:14px;margin-top:10px">결과는 학생의 출발점과 학습량에 따라 달라집니다. 무료 진단에서 ${grade.name} 학생 맞춤 플랜을 받아보세요.</p>
</div></section>`;
})()}
...
<div class="step"><div class="n">2</div><h3>선생님 안내</h3><p>진단 결과와 아이 성향, 지역 일정에 맞는 선생님을 연결합니다.</p></div>
...
<div class="step"><div class="n">2</div><h3>선생님 안내</h3><p>${pick([`${esc(name)} 또는 인근 학교 수업 경험자를 우선 연결합니다.`, `진단 결과와 아이 성향에 맞는 선생님을 안내해 드립니다.`, `${esc(region)} 방문·화상 가능한 선생님을 찾아드려요.`], sd, 96)}</p></div>
...
<p class="sub" style="max-width:760px;font-size:14px;margin-top:10px">결과는 학생의 출발점과 학습량에 따라 달라집니다. 무료 진단에서 ${esc(name)} 학생 맞춤 플랜을 받아보세요.</p>
</div></section>`;
})()}
