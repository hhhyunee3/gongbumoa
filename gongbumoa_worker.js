  ...
  <div class="wrap">
    ...
    <div class="cat-grid">
      ...
      <a href="/others" class="cat cat-4 reveal">
        <span class="cat-ic">🎨</span>
        <h3>기타수업</h3>
        <p>논술·면접·화상 과외 등 목표에 맞춘 다양한 특별 수업.</p>
...
function comingSoonPage(kind, url) {
  const map = {
    ...
    'others': { h1: '기타수업',
      lead: '논술·면접 등 목표에 맞춘 특별 수업을 준비하고 있어요.' },
  };
  const v = map[kind];
  const body = `
<section class="hero"><div class="wrap">
...
<h1>목표에 맞춘 특별 수업</h1>
<p class="lead">정규 과목 수업 외에도 논술, 면접·자기소개서, 화상 과외, 검정고시처럼 목표가 분명한 수업을 1:1로 진행합니다. 어떤 수업이 맞는지 모르셔도 괜찮아요. 상담에서 아이 상태를 듣고 함께 정합니다.</p>
...
<p class="sub" style="max-width:780px">특별 수업은 목표가 분명한 만큼 첫 상담이 중요합니다. 아이의 현재 상태와 목표 시점(면접일, 시험일)을 듣고, 어떤 순서로 진행할지 계획을 먼저 세운 뒤 선생님을 연결해 드립니다.</p>
<p class="sub" style="max-width:780px">정규 과목 수업을 받고 있는 학생은 같은 선생님이 이어서 맡을 수도 있고, 논술이나 면접처럼 전문 분야는 해당 경력의 선생님을 별도로 안내해 드립니다.</p>
</div></section>

${faqBlock([
  { q: '정규 과목 수업 없이 논술이나 면접만 받을 수 있나요?', a: '네. 논술, 면접·자기소개서처럼 목표가 분명한 수업은 따로 받을 수 있습니다. 다만 시험 기간에만 짧게 받는 단기 수업은 운영하지 않습니다. 상담에서 목표를 알려주세요.' },
  { q: '수업 횟수와 비용은 어떻게 정해지나요?', a: '목표와 진행 방식에 따라 다릅니다. 상담에서 계획을 세운 뒤 횟수와 비용을 안내해 드리고, 확인 후 시작합니다.' },
...
${ctaBlock('특별 수업')}`;
  ...
  return page({
    title: `기타수업 - 논술·면접·화상 과외·검정고시 1:1 | ${SITE.name}`,
    desc: `논술, 면접·자기소개서, 화상 과외, 검정고시까지 목표에 맞춘 1:1 특별 수업. 상담에서 아이 상태를 듣고 계획을 함께 세웁니다.`,
    canonical: url,
    crumb: crumbs(BC),
    jsonld: crumbLd(BC),
    body,
